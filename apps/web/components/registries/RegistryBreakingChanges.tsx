import * as CossCard from "@/components/ui/card";
import * as CossCollapsible from "@/components/ui/collapsible";
...
type RegistryBreakingChangesCardProps = {
  registry: any;
};

export const RegistryBreakingChangesCard: React.FC<
  RegistryBreakingChangesCardProps
> = ({ registry }) => {
  return (
    <div className="border rounded-md border-border/70 p-1">
      <CossCard.Card className="rounded-sm bg-muted/20">
        <CossCard.CardHeader>
          <div className="flex items-center gap-x-3">
            <RegistryLogo />
            <GithubInfo starsCount={registry.repo?.stars || 0} />
            <Separator orientation="vertical" className="bg-gray-100" />
            <RegistrySyncStatus />
            <Separator orientation="vertical" className="bg-gray-100" />
            <RegistryInventoryStats componentsCount={registry.item || 0} />
            <Separator orientation="vertical" className="bg-gray-100" />
            <RegistryChanges />
          </div>
          <CossCard.CardTitle className="mt-4">
            {registry.name}
          </CossCard.CardTitle>
          <CossCard.CardDescription>
        ...
        <CossCard.CardFooter className="justify-between">
...
const GithubInfo: React.FC<{ starsCount: number }> = ({ starsCount }) => {
  return (
    <div className="flex items-center gap-x-1">
      ...
      />
      <span className="text-xs">{Intl.NumberFormat().format(starsCount)} </span>
    </div>
  );
};

const RegistryInventoryStats: React.FC<{ componentsCount: number }> = ({
  componentsCount,
}) => {
  return (
    <div className="flex items-center gap-x-1">
      <HugeiconsIcon icon={Box} size={12} />
      <span className="text-xs">{componentsCount} components</span>
    </div>
  );
};
...
const RegistryChangesList: React.FC = () => {
  return (
    <CossFrame.Frame>
      <CossFrame.FrameHeader>
        <CossFrame.FrameTitle className="inline-flex items-center justify-between font-medium text-sm cursor-pointer">
          What's changed?
          <HugeiconsIcon icon={ExternalLink} size={12} />
